    ...
    implicit class StringOps(s: String) {
      ...
      def toZonedDateTime: ZonedDateTime = ZonedDateTime.parse(s, datetimeformatter)
        ...
        */
        ...
        get("search-books" :: paramOption("query") :: paramOption("earliest-publish-year").map(_.map(_.toBigDecimal)) :: paramOption("latest-publish-year").map(_.map(_.toBigDecimal)) :: paramOption("min-rating").map(_.map(_.toBigDecimal)) :: paramOption("max-rating").map(_.map(_.toBigDecimal)) :: paramOption("genres") :: paramOption("authors") :: paramOption("isbn") :: paramOption("oclc") :: paramOption("sort") :: paramOption("sort-direction") :: paramOption("offset").map(_.map(_.toBigDecimal)) :: paramOption("number").map(_.map(_.toBigDecimal)) :: param("api-key") :: header("x-api-key")) { (query: Option[String], earliestPublishYear: Option[BigDecimal], latestPublishYear: Option[BigDecimal], minRating: Option[BigDecimal], maxRating: Option[BigDecimal], genres: Option[String], authors: Option[String], isbn: Option[String], oclc: Option[String], sort: Option[String], sortDirection: Option[String], offset: Option[BigDecimal], number: Option[BigDecimal], authParamapiKey: String, authParamheaderApiKey: String) =>
          da.Default_searchBooks(query, earliestPublishYear, latestPublishYear, minRating, maxRating, genres, authors, isbn, oclc, sort, sortDirection, offset, number, authParamapiKey, authParamheaderApiKey) match {
            case Left(error) => checkError(error)
            case Right(data) => Ok(data)
          }
        } handle {
